...
- The importance of including input formats, example queries, edge cases, and boundary
  explanations into tool descriptions
...
- Restricting each subagent's toolset to those relevant to its role, preventing
...
- MCP resources a same chanism for exposing content catalogs (e.g., issuesummaries,
...
- Using Read to load full file contents followed by Write when Edit cannot find unique
  anchor text
- Building codebase understanding incrementally: starting with Grep to find entrypoints,
